...
function uploadlinks($key=null){
  ...
  function remove_downloadlink($key){
    ...
    private function getUser($key)
    {
        ...
        if(is_a($this->storage->getAdapter(), 'League\Flysystem\Adapter\Local'))
        {
            $destination = realpath($destination);
            if(is_dir($destination)) $destination = realpath($destination);
            $besitzer = posix_getpwuid(fileowner($destination))['name'];

            if($this->FileDB === null) return $besitzer;
        }
    ...
    public function getDirectoryCollection(string $path, bool $recursive = false): DirectoryCollection
    {
        ...
        foreach ($this->storage->listContents($this->applyPathPrefix($path), $recursive) as $entry) {
            ...
            $user = $this->getUser($entry['path']);
            $share = null;
            $destination = realpath($this->joinPaths($this->storage->getAdapter()->getPathPrefix(), $this->applyPathPrefix($userpath), $name));

            if($entry['type'] == 'file') $share = array_search($destination, $this->downloadlinks());
            if($entry['type'] == 'dir')  $share = array_search($destination, $this->uploadlinks());
